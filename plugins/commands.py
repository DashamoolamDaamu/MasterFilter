...
from utils.stream_utils import gen_stream_links
import re
import json
...
async def send_file_to_user(client, user_id, file_id, protect_content_flag, file_name=None, file_size=None, file_caption=None, requester_mention=None):
    try:
        ...
        if FILE_CHANNEL_SENDING_MODE and FILE_CHANNELS:
            ...
            # Direct send — capture the sent message so we can reply with stream buttons
            ...
            # ── Step 2: attach Stream + Download buttons (if streaming configured) ──
...
async def _attach_stream_buttons(client, sent_msg, file_id: str):
    ...
    Silently skips if STREAM_BASE_URL or BIN_CHANNEL is not configured.
    """
    ...
    try:
        ...
        logger.warning(f"Could not attach stream buttons: {e}")
